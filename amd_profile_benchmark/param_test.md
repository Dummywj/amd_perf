...
- 指令测试必须保存反汇编。容量测试扫描阻塞指令数，使用性能曲线拐点给出区间。
...
| `src/instruction_throughput.cpp` | 多条独立寄存器链 | issue interval、512 位拆分、FP/整数/shuffle/conversion 的有效容量 |
| `src/resource_contention.cpp` | 两类指令按 1:1 混合并与单流比较 | opcode 资源重叠、`resource_choices`、有效 occupancy |
...
分别测 opcode A、B 的单流，再测 1:1 混合流。如果混合吞吐显著低于两条独立瓶颈可达到的吞吐，则二者共享资源。没有逐 pipe PMC 时只保存等价资源组，不声明物理 pipe 编号。

覆盖的代表指令为：FP `vaddps`/`vfmadd231ps`、vector integer `vpaddd`、basic shuffle `vpermps`、conversion `vcvtps2dq`。
...
- 反汇编不包含目标指令、工作集未落入预期层级、PMU multiplex 运行比例过低或重复结果 CV 超过 3% 时，结果标为不稳定。
