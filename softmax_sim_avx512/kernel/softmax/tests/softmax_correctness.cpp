...
bool CheckCase(std::size_t count, InputCase input_case) {
  ...
  softmax_avx512_f32(input.data(), vectorized.data(), count);
  ...
  const bool passed = approximation_error <= 1.0e-5f &&
                      vector_error <= 2.0e-6f &&
                      std::abs(output_sum - 1.0) <= 2.0e-5;
  std::printf(
      "n=%zu case=%s approx_abs=%g vector_abs=%g sum=%.9f %s\n", count,
...
int main() {
  bool passed = true;
  constexpr std::size_t kCounts[] = {16, 64, 256, 4096};
