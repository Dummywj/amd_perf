...
from .common import ExecutionModel, SimulationResult, SimulatorError
from ..simulator.memory import CacheMode, MemoryHierarchy
...
from ..simulator.semantic import semantic_id
from ..simulator.trace import validate_bound_trace

if TYPE_CHECKING:
    from ..simulator.profile import Profile


class XsaiBackendError(ValueError):
...
class XsaiBackendUop:
    ...
    ``execution_uop_ids`` links the scheduler entry to the existing, profile-
    timed execution operations.  An empty tuple denotes an internal decode uop
    whose timing is not yet independently calibrated; it still consumes one
    scheduler slot.  This separation avoids inventing latency parameters while
    making XSAI's real backend-uop count observable.
    """
    ...
    @property
    def owns_execution_timing(self) -> bool:
        return bool(self.execution_uop_ids)


@dataclass(frozen=True)
class XsaiBackendTrace:
    profile_id: str
    uops: tuple[XsaiBackendUop, ...]
    macro_uop_ids: dict[str, tuple[str, ...]]

    def uops_for_macro(self, macro_id: str) -> tuple[XsaiBackendUop, ...]:
        ids = set(self.macro_uop_ids[macro_id])
        return tuple(uop for uop in self.uops if uop.id in ids)

    @property
    def scheduler_slot_count(self) -> int:
...
class _PendingUop:
    ...
    slot_count: int
    semantic_ids: tuple[str, ...]
    execution_uops: tuple[ExecutionUop, ...]
    scheduler_partition_choices: tuple[str, ...]
    execution_unit_choices: tuple[str, ...]
    resource_choices: tuple[str, ...]
    execution_dependencies: set[str]
    local_dependencies: set[str]
    part_index: int | None = None
    part_count: int | None = None


class _XsaiUopExpander:
    """Expand architectural RVV instructions into XSAI scheduler entries.

    It intentionally does not borrow the Zen4 engine. Internal entries without
    independently calibrated timing remain structural scheduler slots; the
    calibrated aggregate timing stays on the final entry.
    """

    def __init__(self, trace: BoundTrace, profile: Profile):
        validate_bound_trace(trace)
        if trace.profile_id != profile.id:
            raise XsaiBackendError(
                "bound trace/profile mismatch: "
                f"{trace.profile_id!r} != {profile.id!r}"
            )
        self.trace = trace
        self.profile = profile
        self.backend_trace = self._expand_backend_uops()

    def scheduler_slot_demand(self, macro_id: str) -> int:
        return len(self.backend_trace.macro_uop_ids[macro_id])

    def _expand_backend_uops(self) -> XsaiBackendTrace:
        execution_by_id = {uop.id: uop for uop in self.trace.uops}
        instruction_by_id = {
            str(instruction["id"]): instruction
            for instruction in self.trace.source_trace.get("instructions", [])
        }
        pending: list[_PendingUop] = []
        macro_uop_ids: dict[str, tuple[str, ...]] = {}
        execution_owner: dict[str, str] = {}

        for macro in self.trace.macros:
            instruction = instruction_by_id.get(macro.id)
            if instruction is None:
                raise XsaiBackendError(
                    f"bound macro {macro.id} has no source instruction"
                )
            execution = tuple(execution_by_id[uop_id] for uop_id in macro.uop_ids)
            groups = self._groups_for_macro(macro, instruction, execution)
            ids: list[str] = []
            previous_synthetic: str | None = None
            for slot_index, (role, group, synthetic_chain) in enumerate(groups):
                # Preserve an existing execution-uop id whenever the backend
                # slot owns one. Purely structural slots receive an XSAI id.
                # This keeps semantic visualizers and trace consumers stable
                # while still exposing every physical scheduler entry.
                backend_id = (
                    group[-1].id if group else f"{macro.id}.xb{slot_index}"
                )
                ids.append(backend_id)
                for uop in group:
                    if uop.id in execution_owner:
                        raise XsaiBackendError(
                            f"execution uop {uop.id} has multiple backend owners"
                        )
                    execution_owner[uop.id] = backend_id
                semantic_ids = _ordered_unique(
                    semantic_id
                    for uop in (group or execution)
                    for semantic_id in uop.semantic_ids
                )
                local_dependencies = (
                    {previous_synthetic}
                    if synthetic_chain and previous_synthetic is not None
                    else set()
                )
                pending.append(
                    _PendingUop(
                        id=backend_id,
                        sequence=len(pending),
                        parent_id=macro.id,
                        role=role,
                        slot_index=slot_index,
                        slot_count=len(groups),
                        semantic_ids=semantic_ids,
                        execution_uops=group,
                        scheduler_partition_choices=_ordered_unique(
                            value
                            for uop in (group or execution)
                            for value in uop.scheduler_partition_choices
                        ),
                        execution_unit_choices=_ordered_unique(
                            value
                            for uop in (group or execution)
                            for value in uop.execution_unit_choices
                        ),
                        resource_choices=_ordered_unique(
                            value
                            for uop in (group or execution)
                            for value in uop.resource_choices
                        ),
                        execution_dependencies={
                            dependency for uop in group for dependency in uop.dependencies
                        },
                        local_dependencies=local_dependencies,
                        part_index=_single_value(
                            (uop.part_index for uop in group), "part_index", macro.id
                        ),
                        part_count=_single_value(
                            (uop.part_count for uop in group), "part_count", macro.id
                        ),
                    )
                )
                if synthetic_chain:
                    previous_synthetic = backend_id
            macro_uop_ids[macro.id] = tuple(ids)

        result: list[XsaiBackendUop] = []
        for entry in pending:
            group_ids = {uop.id for uop in entry.execution_uops}
            translated = set(entry.local_dependencies)
            for dependency in entry.execution_dependencies - group_ids:
                try:
                    translated.add(execution_owner[dependency])
                except KeyError as error:
                    raise XsaiBackendError(
                        f"backend uop {entry.id} references unknown execution "
                        f"dependency {dependency}"
                    ) from error
            translated.discard(entry.id)
            result.append(
                XsaiBackendUop(
                    id=entry.id,
                    ...
                    execution_uop_ids=tuple(
                        uop.id for uop in entry.execution_uops
                    ),
                    scheduler_partition_choices=entry.scheduler_partition_choices,
                    execution_unit_choices=entry.execution_unit_choices,
                    resource_choices=entry.resource_choices,
                    dependencies=frozenset(translated),
                    part_index=entry.part_index,
                    part_count=entry.part_count,
                )
            )
        return XsaiBackendTrace(
            profile_id=self.trace.profile_id,
            uops=tuple(result),
            macro_uop_ids=macro_uop_ids,
        )

    def _groups_for_macro(
        self,
        macro: MacroOp,
        instruction: dict,
        execution: tuple[ExecutionUop, ...],
    ) -> list[tuple[str, tuple[ExecutionUop, ...], bool]]:
        semantic_kinds = {
            str(semantic["kind"])
            for semantic in instruction.get("semantic_uops", [])
        }
        if "vector_config" in semantic_kinds:
            return self._synthetic_sequence("vector-config", macro, execution)
        if any(kind.startswith("vector_reduce_") for kind in semantic_kinds):
            return self._synthetic_sequence("vector-reduction", macro, execution)
        if instruction.get("memory") is not None and semantic_kinds.intersection(
            {"address_generation", "vector_load", "vector_store"}
        ):
            return self._vector_memory_groups(macro, instruction, execution)
        if instruction.get("vector_state") is not None and any(
            kind.startswith("vector_") for kind in semantic_kinds
        ):
            return self._vector_compute_groups(macro, instruction, execution)
        return [
            (f"scalar-{uop.kind}", (uop,), False) for uop in execution
        ]
    ...
    def _synthetic_sequence(
        ...
        if not execution:
            raise XsaiBackendError(f"{macro.id}: {role} has no execution uop")
        count = macro.decoded_macro_ops
        if count < 1:
            raise XsaiBackendError(f"{macro.id}: invalid decoded uop count {count}")
        # Only the final scheduler uop owns the calibrated aggregate execution
        # timing. Earlier entries are explicit scheduler slots, not zero-cycle
        # execution claims.
        return [
            (
                f"{role}-{index}",
                execution if index == count - 1 else (),
                True,
            )
            for index in range(count)
        ]

    def _vector_compute_groups(
        self,
        macro: MacroOp,
        instruction: dict,
        execution: tuple[ExecutionUop, ...],
    ) -> list[tuple[str, tuple[ExecutionUop, ...], bool]]:
        prep = tuple(uop for uop in execution if uop.part_index is None)
        parts = _groups_by_part(uop for uop in execution if uop.part_index is not None)
        expected_parts = _lmul_parts(instruction["vector_state"].get("lmul"))
        if len(parts) != expected_parts:
            raise XsaiBackendError(
                f"{macro.id}: expected {expected_parts} LMUL scheduler uops, "
                f"found {len(parts)}"
            )
        groups = [
            ("vector-scalar-prep", (uop,), False) for uop in prep
        ]
        groups.extend(
            ("vector-lmul-part", group, False) for _, group in parts
        )
        return groups

    def _vector_memory_groups(
        self,
        macro: MacroOp,
        instruction: dict,
        execution: tuple[ExecutionUop, ...],
        ...
        prep = tuple(uop for uop in execution if uop.part_index is None)
        flows = _groups_by_part(uop for uop in execution if uop.part_index is not None)
        active_bits = int(
            instruction.get(
                "active_vector_bits", int(instruction["memory"]["bytes"]) * 8
            )
        )
        register_width = int(
            self.profile.backend["vector_decomposition"]["register_width_bits"]
        )
        expected_flows = max(1, (active_bits + register_width - 1) // register_width)
        if len(flows) != expected_flows:
            raise XsaiBackendError(
                f"{macro.id}: expected {expected_flows} EMUL/memory flows, "
                f"found {len(flows)}"
            )
        groups = [("vector-memory-prep", (uop,), False) for uop in prep]
        groups.extend(("vector-memory-flow", group, False) for _, group in flows)
        return groups


def _groups_by_part(
    uops: Iterable[ExecutionUop],
) -> list[tuple[int, tuple[ExecutionUop, ...]]]:
    grouped: dict[int, list[ExecutionUop]] = {}
    for uop in uops:
        if uop.part_index is None:
            raise XsaiBackendError(f"uop {uop.id} has no vector part index")
        grouped.setdefault(uop.part_index, []).append(uop)
    return [
        (part_index, tuple(grouped[part_index]))
        for part_index in sorted(grouped)
    ]


def _lmul_parts(value: object) -> int:
    token = str(value or "m1")
    if token.startswith("mf"):
        return 1
    if token.startswith("m") and token[1:].isdigit():
        return max(1, int(token[1:]))
    raise XsaiBackendError(f"invalid RVV LMUL value: {value!r}")
...
def _single_value(
    values: Iterable[int | None], field: str, macro_id: str
) -> int | None:
    unique = set(values)
    if not unique:
        return None
    if len(unique) != 1:
        raise XsaiBackendError(
            f"{macro_id}: grouped execution uops disagree on {field}"
        )
    return unique.pop()


def _materialize_xsai_trace(
    trace: BoundTrace, backend_trace: XsaiBackendTrace
) -> BoundTrace:
    """Create the scheduler-visible trace consumed by the XSAI timing loop."""
    source_uops = {uop.id: uop for uop in trace.uops}
    source_macros = {macro.id: macro for macro in trace.macros}
    owner_by_execution = {
        execution_id: backend_uop.id
        for backend_uop in backend_trace.uops
        for execution_id in backend_uop.execution_uop_ids
    }
    source_by_macro = {
        macro.id: tuple(source_uops[uop_id] for uop_id in macro.uop_ids)
        for macro in trace.macros
    }
    materialized: list[ExecutionUop] = []

    for backend_uop in backend_trace.uops:
        linked = tuple(
            source_uops[uop_id] for uop_id in backend_uop.execution_uop_ids
        )
        candidates = linked or source_by_macro[backend_uop.parent_id]
        owner = next(
            (
                uop
                for uop in reversed(candidates)
                if uop.kind in {"load_data", "store_data"}
            ),
            candidates[-1],
        )
        uop = copy.deepcopy(owner)
        uop.id = backend_uop.id
        uop.sequence = len(materialized)
        uop.semantic_ids = backend_uop.semantic_ids
        uop.semantic_kinds = _ordered_unique(
            kind for candidate in candidates for kind in candidate.semantic_kinds
        )
        uop.dependencies = set(backend_uop.dependencies)
        uop.vector_state_dependencies = {
            owner_by_execution[value]
            for candidate in linked
            for value in candidate.vector_state_dependencies
            if value in owner_by_execution
            and owner_by_execution[value] != backend_uop.id
        }
        uop.old_destination_dependencies = {
            owner_by_execution[value]
            for candidate in linked
            for value in candidate.old_destination_dependencies
            if value in owner_by_execution
            and owner_by_execution[value] != backend_uop.id
        }
        uop.scheduler_partition_choices = backend_uop.scheduler_partition_choices
        uop.execution_unit_choices = backend_uop.execution_unit_choices
        uop.resource_choices = owner.resource_choices
        uop.part_index = backend_uop.part_index
        uop.part_count = backend_uop.part_count
        uop.issue_after_uop = (
            owner_by_execution.get(owner.issue_after_uop)
            if owner.issue_after_uop is not None
            else None
        )
        if uop.issue_after_uop == uop.id:
            uop.issue_after_uop = None
        if not backend_uop.owns_execution_timing:
            uop.scheduling_class = f"xsai-internal:{backend_uop.role}"
            uop.latency_ticks = 0
            uop.issue_interval_ticks = 0
            uop.occupancy_ticks = trace.ticks_per_cycle
            uop.rename_allocations = {}
            uop.memory = None
        uop.dispatch_tick = None
        uop.ready_tick = None
        uop.issue_tick = None
        uop.complete_tick = None
        uop.resource = None
        uop.resource_lane = None
        uop.scheduler_partition = None
        uop.execution_unit = None
        uop.vector_read_domain = None
        uop.completion_domain = None
        uop.memory_level = None
        uop.stall_reason = None
        uop.stall_reasons = {}
        materialized.append(uop)

    macros: list[MacroOp] = []
    for source in trace.macros:
        macro = copy.deepcopy(source)
        macro.uop_ids = backend_trace.macro_uop_ids[source.id]
        macro.dispatch_units = len(macro.uop_ids)
        macro.dispatch_tick = None
        macro.complete_tick = None
        macro.retire_tick = None
        macro.dispatch_blocker = None
        macro.dispatch_blocker_domain = None
        macro.dispatch_blocker_count = 0
        macros.append(macro)

    result = copy.deepcopy(trace)
    result.macros = macros
    result.uops = materialized
    validate_bound_trace(result)
    return result


def _passthrough_backend_trace(trace: BoundTrace) -> XsaiBackendTrace:
    """Represent a legacy generic-token trace without ISA-specific expansion."""
    execution_by_id = {uop.id: uop for uop in trace.uops}
    entries: list[XsaiBackendUop] = []
    for macro in trace.macros:
        for index, uop_id in enumerate(macro.uop_ids):
            uop = execution_by_id[uop_id]
            entries.append(
                XsaiBackendUop(
                    id=uop.id,
                    sequence=uop.sequence,
                    parent_id=uop.parent_id,
                    role=f"generic-{uop.kind}",
                    slot_index=index,
                    slot_count=len(macro.uop_ids),
                    semantic_ids=uop.semantic_ids,
                    execution_uop_ids=(uop.id,),
                    scheduler_partition_choices=uop.scheduler_partition_choices,
                    execution_unit_choices=uop.execution_unit_choices,
                    resource_choices=uop.resource_choices,
                    dependencies=frozenset(uop.dependencies),
    ...
    return XsaiBackendTrace(
        ...
        uops=tuple(entries),
        macro_uop_ids={macro.id: macro.uop_ids for macro in trace.macros},
    )
...
class XsaiRvvEngine:
    ...
    def __init__(
        ...
        *,
        expand_backend_uops: bool = True,
    ):
        if execution_model not in {"out_of_order", "in_order"}:
            raise SimulatorError(f"unknown execution model: {execution_model}")
        validate_bound_trace(trace)
        if expand_backend_uops and trace.source_trace.get("trace_version") == 2:
            self.backend_trace = _XsaiUopExpander(trace, profile).backend_trace
            trace = _materialize_xsai_trace(trace, self.backend_trace)
        else:
            self.backend_trace = _passthrough_backend_trace(trace)
    ...
    def run(self) -> SimulationResult:
        ...
        if self.backend_id == "xsai-rvv":
            summary["xsai_backend"] = {
                "scheduler_uops": self.backend_trace.scheduler_slot_count,
                "roles": dict(
                    sorted(
                        Counter(uop.role for uop in self.backend_trace.uops).items()
                    )
                ),
            }
...
class GenericTokenEngine(XsaiRvvEngine):
    """Compatibility engine for explicit legacy generic-token profiles."""

    backend_id = "generic-token"
